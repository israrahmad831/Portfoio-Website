...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      ...
      {/* Mobile Navigation */}
      <div
        className={`md:hidden fixed top-24 left-1/2 transform -translate-x-1/2 transition-all duration-500 ease-out ${
          isOpen
            ? "opacity-100 translate-y-0 scale-100"
            : "opacity-0 -translate-y-8 scale-95 pointer-events-none"
        }`}
      >
        <div className="relative bg-gradient-to-br from-gray-900/95 via-gray-800/95 to-gray-900/95 backdrop-blur-xl rounded-3xl border border-gray-700/50 shadow-2xl p-3 min-w-[220px]">
          <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-pink-500/10 blur-2xl"></div>
          ...
          {navLinks.map((link, index) => (
            <a
              ...
              style={{
                ...
                animation: isOpen
                  ? `slideInLeft 0.4s ease-out ${index * 60}ms both`
                  : "none",
              }}
              className={`relative block px-6 py-3.5 rounded-2xl text-base font-semibold transition-all duration-300 mb-2 last:mb-0 ${
                activeSection === link.name.toLowerCase()
                  ? "bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg scale-105"
                  : "text-gray-300 hover:text-white hover:bg-gray-700/50 hover:scale-105"
              }`}
            >
              <span className="relative z-10">{link.name}</span>
              {activeSection === link.name.toLowerCase() && (
                <span className="absolute inset-0 rounded-2xl bg-gradient-to-r from-blue-400 to-purple-500 blur-md opacity-40"></span>
              )}
            </a>
          ))}
        </div>
